...
function warning(){
  ...
  if (nb_checked > 2 || (nb_checked > 1 && adversary_type == 'minion'))
    warnings.push('Выбрано слишком много вариантов защиты.');

  if (result['defense']['soak'] > 7)
    warnings.push('Поглощение не должно быть выше 7.');


  if (result['defense']['melee_defense'] > 4 || result['defense']['ranged_defense'] > 4)
    warnings.push('Рейтинг защиты не должен превышать 4.');
  ...
  if (Object.values(result['skills']).length > 8)
    warnings.push('Максимальное рекомендуемое количество навыков — 8; Вы можете отказаться от любых ненужных.');
